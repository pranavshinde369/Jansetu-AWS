...
- **Voice_Processor**: Component handling audio input/output and speech recognition/synthesis via Bhashini API
- **Scheme_Agent**: AI reasoning component powered by Amazon Bedrock (Claude 3.5 Sonnet) that identifies appropriate government schemes
- **Form_Generator**: Component that creates pre-filled PDF application forms using PyPDF2
- **Scheme_Database**: JSON-based storage for government scheme information
...
1. WHEN a user speaks in any supported Indian dialect, THE Voice_Processor SHALL detect the language automatically using Bhashini API
2. WHEN language detection is uncertain, THE Voice_Processor SHALL ask the user to confirm their preferred language
3. THE Voice_Processor SHALL support Hindi, Bhojpuri, and at least 5 other major Indian languages through Bhashini
4. WHEN processing dialect-specific terms, THE Scheme_Agent SHALL maintain context and meaning accuracy
5. WHEN responding to users, THE Scheme_Agent SHALL generate responses in the same dialect as the input
6. WHEN a dialect is not supported, THE Voice_Processor SHALL inform the user and offer Hindi as an alternative
...
1. WHEN a user describes their situation, THE Scheme_Agent powered by Amazon Bedrock SHALL analyze their needs against available government schemes in the JSON database
...
4. WHEN scheme information is retrieved, THE Scheme_Database SHALL provide accurate and up-to-date scheme details
...
1. WHEN the Scheme_Agent generates responses, THE Guardrail_System SHALL verify information against the JSON Scheme_Database
...
4. WHEN asked about information not in the Scheme_Database, THE Guardrail_System SHALL direct users to official government sources
...
1. WHEN user information is collected, THE Form_Generator SHALL create a pre-filled PDF application form using PyPDF2 with coordinate-based field positioning
...
1. WHEN collecting user data, THE JanSetu SHALL encrypt all personal information in transit using HTTPS/TLS
...
### Requirement 9: AWS Deployment and Infrastructure

**User Story:** As a system administrator, I want the application deployed securely on AWS infrastructure, so that it provides reliable 24/7 service with proper security controls.

#### Acceptance Criteria

1. WHEN the backend is deployed, THE JanSetu SHALL run on AWS EC2 instance with FastAPI and Uvicorn
2. THE JanSetu SHALL use Amazon Bedrock (Claude 3.5 Sonnet) for AI reasoning via secure IAM credentials
3. WHEN storing sensitive credentials, THE JanSetu SHALL use environment variables in .env file that are never committed to version control
4. THE JanSetu SHALL maintain 24/7 uptime using detached screen sessions or process managers
5. WHEN accessing AWS services, THE JanSetu SHALL use IAM roles and policies with least privilege access
6. THE JanSetu SHALL log all API calls and errors for monitoring and debugging